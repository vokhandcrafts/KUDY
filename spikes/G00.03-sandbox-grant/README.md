...
```bash
# аўтаматычныя праверкі спайку (node:test, loopback, без сеткі)
npm test
...
- наступствы: RC-адаптар не правяраўся па сетцы; да жывой звяркі ў G00.03.c
  трэба пацвердзіць форму адказу, чытанне sandbox/production атрыбуцыі і
  трактоўку 404 (спайку лічыць яе subscriber-not-found → «не купляў»); ніякі
  тэст тут не выдае сябе за праверку рэальнага store/OS.
