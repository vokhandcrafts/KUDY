...
`isAboveAdvisoryRange` цяпер рэжа спекку па `||` і правярае кожную частку
асобна (`matchAll` з сягам `g`): спека праходзіць толькі калі кожны
дазволены ёю трыпл вышэй дыяпазону. Частка без версійнага трыпла —
dist-tag (`latest`, `next`) або шаблон (`8.5.x`) — дазваляе версіі, якіх
праверка не пералічвае, таму рэжацца loudly з іменаванай дыягностыкай
(`no version triple` / `wildcards are not supported`, шаблон пазнавацца
толькі ў пазіцыі нумара версіі). `web/package.json` і lockfile не
кранаюцца: override застаецца `^8.5.23`, resolved — 8.5.28.
...
жах новых тэстах падаюць роўна два — «every version triple» (на
`^8.5.23 || 8.4.31`, які стары код прымае) і «cannot fully enumerate» (на
`latest`, `^8.5.23 || latest`, `^8.5.23 || next`, `8.5.x || ^8.5.23`, якія
стары код ціха прымае або сустракае з непрыдатнай дыягностыкай);
`# pass 2 / # fail 2`. Аднаўленне меры — зноў `# pass 4 / # fail 0`.
