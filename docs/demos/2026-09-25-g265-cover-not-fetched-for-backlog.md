# G17.05 follow-up — the cover is not fetched for an asr-backlog video

*Showboat demo for judge-finding #265 (`tools/collector/youtube.mjs`), created 2026-09-25.*

<!-- showboat-id: g265-cover-not-fetched-for-backlog -->

`youtubeFetch` loads the thumbnail only when a subtitle file was actually
selected — a video without usable subtitles goes to the asr-backlog and must
not cost a network request. Live YouTube fetches are manual (no network here):
the demo drives the production run loop with the yt-dlp binary behind its
`command` boundary replaced by a stub that emits bundled fixtures, and the
cover image is served by a local fixture server on port 8083 that logs every
request. The request log is the counter: exactly one cover request arrives —
for the collected video; the backlog video contributes none.

Setup: fixtures, campaign (one subtitled video + one subtitle-less, both with
a thumbnail), the stub command, the logging cover server, then the production
`runCampaign`:

```sh
kill $(cat /tmp/kudy-g265-demo/server.pid 2>/dev/null) 2>/dev/null; rm -rf /tmp/kudy-g265-demo && mkdir -p /tmp/kudy-g265-demo && node -e '
const fs = require("node:fs");
const dir = "/tmp/kudy-g265-demo";
const vtt = [
  "WEBVTT", "",
...
const info = {
  ...
  duration: 217.4,
  language: "en",
  thumbnail: "http://127.0.0.1:8083/cover.jpg",
...
fs.writeFileSync(dir + "/campaign.yaml", [
  "city: gdansk",
  "seeds:",
  "  - file:///tmp/kudy-g265-demo/seed.html",
  "topics: [history]",
  "fence:",
  "  depth: 3",
  "  extra_domains: []",
...
const server = http.createServer((req, res) => {
  fs.appendFileSync("/tmp/kudy-g265-demo/requests.log", req.url + "\n");
  res.writeHead(200, { "content-type": "image/jpeg" });
  res.end(Buffer.from([0xff, 0xd8, 0xff, 0xd9]));
});
server.listen(8083, "127.0.0.1", () => {
  fs.writeFileSync("/tmp/kudy-g265-demo/server.pid", String(process.pid));
  console.log("serving cover on 8083");
});
' > /tmp/kudy-g265-demo/server.log 2>&1 < /dev/null & ) ; node --input-type=module -e '
...
const rows = db.prepare("SELECT url, snapshot_path FROM raw_records WHERE source_type = \u0027youtube\u0027 ORDER BY url").all();
for (const row of rows) {
  console.log(`${row.url} | snapshot ${row.snapshot_path ? "written" : "none"}`);
}
...
console.log(`collected video cover: ${metadata.cover}`);
const requests = fs.readFileSync(dir + "/requests.log", "utf8").trimEnd().split("\n");
console.log(`--- cover requests seen by the fixture server: ${requests.length} ---`);
...
https://www.youtube.com/watch?v=aQw4w9WgXcQ | snapshot none
https://www.youtube.com/watch?v=dQw4w9WgXcQ | snapshot written
collected video cover: media/gdansk-shipyard-the-august-story-cover.jpg
--- cover requests seen by the fixture server: 1 ---
GET /cover.jpg
```
...
```sh
kill $(cat /tmp/kudy-g265-demo/server.pid) && echo "fixture server stopped"
```
...
The YouTube collector suite (includes the issue #265 counter test):

```sh
node --test "tools/collector/youtube.test.mjs" 2>/dev/null | grep -E "^ℹ (tests|pass|fail)"
```
