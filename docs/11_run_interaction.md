...
Удакладненні пра меню **Горад → Гіды**, асобныя аўдыё і ручныя Moments — [15](15_guide_decisions.md); рэалізацыя раскладзена ў [16](16_delivery_backlog.md). P01/P02 там пазначаныя як прапановы, а не выкананыя змены мадэлі.
...
| `heard: Set<stop_id>` | манатонны | наратыў хоць раз дайшоў да канца | на `AudioFinished` |
| `playing: stop_id?` | зменлівы | гучыць зараз | на `PlayStory`, здымаецца на любым спыненні |

Статус, які бачыць чалавек, **выводзіцца** з іх у гэтым парадку:

| Статус | Умова | Аўтазапуск адсюль |
|---|---|---|
| `locked` | няма гатовага дазволенага кантэнту | не; толькі адкрытае прэв’ю |
| `playing` | `playing == stop_id` | — |
| `played` | `stop_id ∈ heard` | не |
| `available` | `stop_id ∈ auto_fired` | не |
| `pending` | ні ў адным наборы | **так** |
...
`available` — не памылка і не «прапушчана назаўсёды». Сюды кропка трапляе, калі аўтатрыгер быў выдаткаваны, а гісторыя цалкам не прагучала: выцеснена з чаргі, адкладзены запуск адхілены праз нясвежую пазіцыю, аўтаматычна запушчанае аўдыё спынена рукамі або іншай кропкай. Калі перарвана першае ручное прайграванне без auto_fired/ heard, статус вяртаецца ў pending.

**Ручны паўтор даступнага загружанага кантэнту дазволены заўсёды**, у тым ліку для `played` — кнопка называецца «Паслухаць яшчэ раз». Ён не чапае ні `heard`, ні `auto_fired`; перарваны паўтор проста вяртае кропку да таго статусу, які быў.

**«Яшчэ можна адкрыць»** — даступныя загружаныя кропкі не ў `heard` у момант `Finish`. Гэта магчымасці, не пропускі або памылкі; замкнёныя кропкі не ўваходзяць. `heard` тут апісвае адзін наратыў на кропку. Выбар і незалежны прагрэс base/extended у той самай кропцы — абавязковае адкрытае пытанне тэхнічнага спецыфікавання (`13`, разд. 4).

### 3.2 Што грае і што разглядаюць — розныя рэчы

Два незалежныя паказальнікі:

- **`playing: stop_id?`** (у UI — `nowPlaying`) — чый наратыў гучыць. Ім кіруе плэер-паласа ў Peek і lock-screen.
...
`Finish` спыняе аўдыё, геафенсы і падпіску на пазіцыю, адпускае wakelock, ачышчае чаргу, захоўвае вынік і паказвае экран завяршэння. «Яшчэ можна адкрыць» вылічаецца толькі з даступных загружаных кропак не ў `heard`.
...
4. Кропка мае вылічаны статус `pending`, то бок **не належыць ні `auto_fired`, ні `heard`**. Праверка `heard` тут абавязковая: ручное прайграванне `auto_fired` не чапае, і без яе гісторыя, паслуханая рукамі загадзя, зайграла б яшчэ раз пры падыходзе.
...
| **4** — ужо ў `auto_fired` або ў `heard` | трыгер ігнаруецца (інварыянт «не двойчы» / «ужо чуў») | ужо там або не патрэбны |
...
Ключавое правіла: **`auto_fired` ставіцца толькі там, дзе аўтаматычная спроба сапраўды скончана** — то бок пры фактычным запуску або пры пераводзе ў ручны доступ. Чарга і ігнараванне яго не чапаюць, таму кропка застаецца прыдатнай для наступнага спрацоўвання.
...
**Здымаецца толькі яўным дзеяннем чалавека:** Play / Resume у паласе або на lock-screen · ручное прайграванне любой кропкі · «Працягнуць» пасля паўзы сесіі.
...
- **Прыйшоў другі, іншы stop, пакуль чарга занятая** — у ячэйцы застаецца **найноўшы** (па часе; не гарантыя найменшай адлегласці); выцеснены становіцца `available` і трапляе ў `auto_fired`: яго аўтаматычная спроба скончана, застаецца ручны доступ.
- **Пасля `AudioFinished`** адкладзены трыгер грае толькі калі выкананыя ўмовы 1, 2, 4, 5 разд. 5.1 і чалавек усё яшчэ ў межах `2 × trigger_radius`, а кантэнт па ўмове 6 даступны. Пры запуску кропка трапляе ў `auto_fired`.
- **Калі праверка не прайшла** (нясвежая пазіцыя, чалавек адышоў, аўтаматыка прыпыненая) — кропка пакідае чаргу як `available` + `auto_fired`. Тут спроба сапраўды скончана, у адрозненне ад пастаноўкі ў чаргу.
...
- **Паўза сесіі і `Finish`** ачышчаюць чаргу; кропка з ячэйкі → `available` + `auto_fired`.
...
|---|---|---|
| `playing` | `playing == stop_id` | каманда `PlayStory` |
| `played` | `stop_id ∈ heard` | `AudioFinished` (назаўсёды) |
| `available` | `stop_id ∈ auto_fired`, але не ў `heard` | выцясненне з чаргі, адхілены адкладзены запуск, ручны доступ пры прыпыненай аўтаматыцы |
...
| `auto_fired`, `heard` (манатонныя), `playing` (зменлівы) | `core/engine` + SQLite; наборы перажываюць force-stop |
...
| Яшчэ можна адкрыць | пры `Finish`: даступныя загружаныя кропкі не ў `heard`, без locked |
| Ручное прайграванне і паўтор | той самы `UserSelectedStop`, што і GPS-трыгер; `heard` не сціраецца |
...
7. **Перарваны паўтор не сцірае факт праслухоўвання:** даслухаў кропку 3 → запусціў яе зноў → спыніў на сярэдзіне → яна застаецца `played` і **не** трапляе ў «Яшчэ можна адкрыць».
...
11. На экране завяршэння «Яшчэ можна адкрыць» змяшчае роўна даступныя загружаныя кропкі не ў `heard`; locked не ўваходзіць.
12. Гісторыя, паслуханая рукамі загадзя (`heard`), пры фізічным падыходзе да кропкі **не** запускаецца паўторна.
...
21. Два трыгеры падчас адной гісторыі: грае найноўшы, выцеснены становіцца `available`.
