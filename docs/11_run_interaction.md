...
| `play_id` — ідэнтычнасць запуску ў пары з `session_id` | `core/engine`: расце на кожны `PlayStory` і пішацца write-through у durable `play_seq`, таму працягваецца пасля restart тае самай сесіі ([ADR G01.03](architecture/decisions/G01.03-session-access.md) §3.1); `AudioFinished` з іншай парай `(session_id, play_id)` ігнаруецца |
