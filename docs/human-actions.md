...
### 2026-09-24 — мерж PR #234 (G04.02.b — грант-кліент: рэтраі, заканчэнне URL)
Што зрабіць: праверце і змержуйце PR #234 у main — download-старона `/v1/grant` (`services/download/grant.ts`: дакладная форма запыту з Bearer з порта, порцыі па 20 шляхоў з верыфікаванага lock, свой вынік на кожны код закрытага спісу 19 §3.6, рэтраі 503 з `Retry-After`, re-grant пры сканчэнні URL па дарозе, азначаны офлайн без кідання, нулявыя ўцечкі сакрэтаў; `activate()` цяпер павярхоўна чыннік збою ў дыягностыкі partial). Код-рэвью раўнд 2 ✅ (0 critical · 0 high · 1 medium), суддзя ✅ (👍 на каментары рэвью = можна мержыць); issue #190 закрываецца мержам.
PR: https://github.com/vokhandcrafts/KUDY/pull/234 (issue #190)

### 2026-09-23 — мерж PR #230 (G05.01.b — аўтатрыгер, чарга і P01 прагрэс)
Што зрабіць: праверце і змержуйце PR #230 у main — рэдуктарны слайс G05.01.b (умовы аўтатрыгера ADR G01.01 §4.8, аднаячэечная чарга з уключнымі краткамі адкладзенага запуску, брама `AudioFinished` па пары `(session_id, play_id)`, правілы прагрэсу P01 і «Яшчэ можна адкрыць»). Код-рэвью і дэльты-рэвью ✅, суддзя пацвердзіць у сваім каментары (👍 на каментары рэвью = можна мержыць); issue #198 закрываецца мержам.
PR: https://github.com/vokhandcrafts/KUDY/pull/230 (issue #198)
