...
### 2026-09-24 — мерж PR #235 (G06.09.b — кампазіцыйны корань, стора кантролераў, правілы слаёў)
Што зрабіць: праверце і змержуйце PR #235 у main — кампазіцыйны корань `controllers/createServices(ports)`, стора-хэлпер над zustand 5.0.15 (пін — ADR G00.04 §6), кантролер-узор з node-тэстам без React Native, машынныя правілы `app-no-core` і `controllers-services-type-only` у arch:check. Код-рэвью (раўнд 1 + дэльта) ✅, суддзя ✅ (👍 на каментарах рэвью); issue #209 закрываецца мержам. Пасля мержу адкрыюцца наступнікі па хэндоверу.
PR: https://github.com/vokhandcrafts/KUDY/pull/235 (issue #209)
