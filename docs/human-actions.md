...
### 2026-09-23 — пятая хваля задач дадатку: мерж брыфаў і пазнакі
Што зрабіць: змержуйце PR з брыфамі пятай хвалі (галіна `feature/app-wave-briefs`). `agent:ready` ужо стаіць на #208 — адзінай частцы без Blocked-by. Астатнім частакам #209–#221 пазнаку ставіць пасля закрыцця іх Blocked-by. Прыладных праверак гэтая хваля не робіць (ваша рашэнне): G05.02.c і G05.03.b пакінуць спіс `not-run` клетак для запуску на тэлефоне.
