...
### 2026-09-24 — мерж PR #232 (G05.01.c — уладальнік гуку ў рэдуктары)
Што зрабіць: праверце і змержуйце PR #232 у main — рэдуктарны слайс G05.01.c (варыянты guide/moment у `playing` з жывой паўзай, токены запускаў `{kind, ref, seq}` з адзіным правілам адхілення тэгаваных callback'аў, яўны Play Moment з ачысткай чаргі і «Працягнуць гід», адзіны 10-хвілінны парог пасля званка, Pause/End спыняюць толькі гук гіда, Start ін'ектуе граючы moment). Код-рэвью і суддзя зялёныя (👍 на каментары рэвью = можна мержыць); issue #199 закрываецца мержам.
PR: https://github.com/vokhandcrafts/KUDY/pull/232 (issue #199)
