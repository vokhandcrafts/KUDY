...
### 2026-09-25 — Суддзёвая знаходка #265: смержыць PR #270
Што зрабіць: смержыце PR #270 (squash) — ён выпраўляе суддзёвую знаходку з PR #263: вокладка YouTube спампоўваецца толькі для збраных відэа, відэа з asr-backlog больш не плаціць лішняга сеткавага запыту. Пасля мержу задача #265 закроецца аўтаматычна; зніміце лэйбл `agent:running` з закрытага issue #265.
PR: https://github.com/vokhandcrafts/KUDY/pull/270 (issue #265)
