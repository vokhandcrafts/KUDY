...
Што зрабіць: выканайце на дэв-хосце `sudo pacman -S --needed npm` — сістэмнага npm-бінарніка няма (толькі інтэрактыўныя аліязы), а expo-doctor, eas-cli і agent-скрыпты спаўняюць npm унутрана; праверка `npm -v`. Затым зарэгіструйцеся на https://expo.dev/signup (бясплатны план: 15 EAS-зборак/месяц на кожную платформу) і выканаць `npx eas-cli login` у тэчцы KUDY. Платныя крокі (Apple Developer $99/год, Google Play Console $25, RevenueCat) свядома адкладзеныя — да G08.
PR: спасылка з'явіцца ў справаздачы задачы (mіні-PR engines+докі)
