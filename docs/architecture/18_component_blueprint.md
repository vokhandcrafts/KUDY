...
**Дапаўненне 2026-09-13:** кампаненты discovery/feedback і іх межы зададзеныя ў [21](21_discovery_feedback_architecture.md). Дыяграмы ніжэй — базавы Run-кантур; дадатковы паток мае ўласную схему ў `21`, §2.

**Файлавы ўзровень:** канкрэтныя шляхі, подпісы інтэрфейсаў, уладальнікі стану і сцэнары жыццёвага цыклу — [19_class_and_module_map.md](19_class_and_module_map.md) (2026-09-15); гэтая схема застаецца ўзроўнем кампанентаў і патокаў.
...
Нявырашаныя hardware/API-паводзіны не зачыняюцца новай дыяграмай. Версіі SDK выбраныя ў адзін набор і запіненыя ([ADR G00.04-stack-baseline §6](decisions/G00.04-stack-baseline.md), result G00.04.c: чыстая ўсталяванне, doctor, prebuild android), але натыўны рантайм на прыладзе — not-run; пастаўшчык тайлаў і offline-manager/pack усё яшчэ патрабуюць вынікаў G00.02.c і замеру M0b. API endpoints, шляхі і коды — `09`, раздзел 5; не ствараць дадатковы backend дзеля схемы.
...
## Кампаненты пашырэння

`core/discovery/selectDiscovery` і discovery controller працуюць з public-index; feedback repository/sync/controller — з асобнымі durable-табліцамі і feedback endpoints. Сервер мае target registry, current ratings і mutation deduplication з device FK/RLS; аўтар атрымлівае толькі прыватную агрэгацыю. Схема і кананічныя палі — `21`, §2–§6. Ні адзін новы модуль не валодае Run, GPS, player або entitlement.
