...
- `RouteStop`: **`id` (стабільны, генеруецца адзін раз і не мяняецца ніколі)**, `route_id`, `position`, `place_id`, `access_tier` (`base` | `extended`), `preview` (публічныя назва, месца, анонс), `story_base_id?`, `story_extended_id?`, `trigger_radius_m?` (перакрывае Place), `optional?`.
  ...
  - `id` — **адзіная ідэнтычнасць кропкі** ва ўсёй сістэме: `auto_fired`, `heard`, `queued`, `session.last_stop`, акно геафенсаў, падзеі логу, дасягненні. Гэта прама выпраўляе памылку TourForge (там ідэнтычнасць кропкі = індэкс у масіве, і ўстаўка кропкі цішком перанумароўвае прагрэс усіх карыстальнікаў).
...
- `Moment`: `id`, `place_id`, `story_id` (звычайна тызер), `kind`, `cooldown_min`. Moment — **трэйлер, не поўная гісторыя** (`04`).
...
- `public/catalog.json` — спіс усяго, што існуе: `route_id`, версія, локалі, наяўныя пласты, размеры, `product_id`. **Спіс адкрыты; закрытая — загрузка.**
...
| `DELETE` | `/v1/device` | Выдаленне дадзеных прылады: `devices`-радок, `event_log`, кэш правоў. Пасля `204` кліент выдаляе `device_id`, `device_secret`, лакальную `event_queue` і стан згоды; загружаныя бандлы застаюцца. Правы на кантэнт **не** выдаляюцца (яны ў сторы і ў RevenueCat) | `Bearer <device_secret>` |
...
`Active` трымае: `session_id`, `route`, `version`, `locale`, `accessible_stop_ids` (толькі гатовы дазволены кантэнт), `tier_available`, `auto_fired: Set<stop_id>`, `heard: Set<stop_id>`, `autoplay_suspended: bool`, `playing?: { stop_id, story_id, play_id }` (пара `(session_id, play_id)` — ідэнтычнасць канкрэтнага запуску), `queued?`, `last_fix`, `focus_lost_at?`.

**Статус кропкі не захоўваецца — ён вылічаецца** спачатку паводле доступу: няма гатовага дазволенага наратыву → `locked`; затым `playing` → `playing`; `∈ heard` → `played`; `∈ auto_fired` → `available`; іначай `pending` (`11`, разд. 3.1).

**Чаму два манатонныя наборы замест аднаго `consumed` і замест зменлівага статусу:** `auto_fired` адказвае «ці спрацоўваў аўтатрыгер» і забараняе паўторны аўтазапуск; `heard` адказвае «ці чуў чалавек гісторыю да канца» і дае спіс яшчэ непраслуханых даступных гісторый, без ацэнкі паспяховасці прагулкі. Зменлівае поле статусу тут не працуе: перарваны ручны паўтор ператварыў бы `played` у `available` і зрабіў бы даслуханую кропку прапушчанай.

**Events:** `Start` · `Pause` · `Resume` · `End` · `LocationAccepted` · `DwellCompleted(stop_id)` · `AudioFinished(session_id, play_id)` · `UserSelectedStop(stop_id)` · `UserPausedAudio` · `FocusLoss` · `FocusRegain` · `Timer(id)` · `AccessReady(version, stop_ids[])`.
...
Прапанова пашырэння вылічаецца ў UI паводле `11`, разд. 8, без каманды engine і без абавязковага AudioFinished. `AccessReady` паступае толькі ад сэрвісу пасля сервернага гранта і поўнай праверанай атамарнай загрузкі: тая ж версія, толькі вядомыя stop_id; іначай падзея адхіляецца цалкам. Яна не запускае аўдыё і не мяняе heard/auto_fired; абнаўляе даступнасць і акно геафенсаў.

**Інварыянты (кожны — property-тэст):**

1. Кропка ніколі не запускаецца **аўтаматычна** двойчы (`auto_fired` манатонна расце). Ручны `UserSelectedStop` дазволены для гатовага даступнага кантэнту, у тым ліку пасля `played`.
...
5. `FocusRegain` пазней за 10 хв → не аднаўляем, а закрываем кропку: чалавек фізічна пайшоў далей, і расказ пра будынак праз тры кварталы — гэта скарга, а не фіча.
6. Знешняе перапыненне (званок) — гэта `FocusLoss`, **ніколі не `AudioFinished`**; інакш званок назаўсёды з'ядае кропку.
7. Аўтазапуск немагчымы, калі `autoplay_suspended = true` або стан ≠ `Active`. Сцяг ставяць `UserPausedAudio`, `FocusLoss` і `Pause`; здымае **толькі** яўная каманда карыстальніка (`Resume`, `UserSelectedStop`).
...
**Свабодная хада:** нумар, кірунак і лінія маршруту не абмяжоўваюць трыгер. `Run` — абраная падборка і аўтазапуск пасля Start; `Map` — агляд горада. Прагляд Map не стварае другога ўладальніка геалакацыі. Moments у MVP адкрываюцца рукамі; у «Побач» прапануюцца без аўтагуку. P02 у `15` і G01.02 у `16` вызначаюць наступнае рашэнне пра фон і адзіны ручны плэер. На End паказваюцца толькі даступныя загружаныя кропкі не ў heard як «Яшчэ можна адкрыць»; усе кропкі слухаць неабавязкова. `deviated` і патрабаванне вярнуцца на лінію не будуюцца.

**Мяжа мадэлі прагрэсу:** heard/auto_fired па stop_id і цяперашняя выканальная мадэль апісваюць адзін наратыў на кропку. Base і extended у адным месцы патрабуюць незалежнай ідэнтычнасці праслухоўвання і правіла выбару аўдыё; гэта неабходна вызначыць да рэалізацыі такога пашырэння. Дадатковыя locked-кропкі ўжо апісваюцца асобнымі stop_id.
...
| Табліца | Ключ | Змест |
|---|---|---|
| `session` | `session_id` | `route_id`, `version` (нязменная), `state` (`active`/`paused`/`finished`), `auto_fired[]` і `heard[]` (спісы `RouteStop.id`, не пазіцый), `last_stop_id`, `started_at`, `finished_at?`, `locale`, `tier`. **Тое, чаго няма ні ў Tramio, ні ў TourForge**. Ключ — **прагулка, не маршрут**: паўторны праход стварае новы радок, і гісторыя праходаў у My KUDY становіцца магчымай. Індэкс па `(route_id, state)` дае «незавершаная прагулка гэтага маршруту»; інварыянт — **не больш за адзін `active`/`paused` радок на ўвесь дадатак** (`11`, разд. 4.1) |
...
`app_open` · `explore_option_tap(route_id)` · `route_preview(route_id)` · `purchase_started/succeeded/failed(attempt_id, route_id, tier, reason?)` · `download_started/completed/failed(download_id, attempt_id?, route_id, version, locale, tier, bytes)` · `session_started/resumed/paused/ended(session_id, route_id, version)` · `stop_reached(session_id, stop_id)` · `story_play_started/story_play_failed(session_id, stop_id, story_id, play_id, trigger: gps|manual)` · `story_audio_completed(session_id, stop_id, story_id, play_id)` · `extension_offer_shown/accepted(offer_id, route_id, stop_id?)` · `moment_shown/opened/dismissed(kind)` · `location_stalled(duration_bucket)` · `autotriggers_unavailable(reason)`.

`session_ended` нясе прычыну (`user_stop` / `switch_walk`) і колькасць праслуханых даступных кропак; не азначае ўсе кропкі або фізічнае праходжанне. `stop_reached` — толькі пацверджаная геалакацыяй прысутнасць, ручны Play яе не стварае. `purchase_started` захоўвае offer_id, калі пакупка прыйшла з прапановы. Адно accepted не даказвае аплату. Паказ — толькі фактычная бачнасць.
...
3. **Мяжа доступу вызначаецца access маршруту і tier кантэнту**, не першымі N кропкамі. `free_stop_count` — толькі вылічальны лік для прэв’ю. Выбар і незалежны прагрэс base/extended адной кропкі патрабуюць асобнага кантракту (`13`).
...
7. **Закрыта: версія сесіі замацаваная да End** (разд. 4, 7). Серверны тэрмін даступнасці старых версій яшчэ трэба задаць у эксплуатацыйным плане.
...
Controller падказак трымае ліміты фактычнага паказу і адхілення па guide_id, без аўдыёкаманд. Session-scope shown/dismissed захоўваецца ў durable-зоне разам з прагрэсам; foreground-scope без сесіі і лакальны last_shown_at падтрымліваюць cooldown. Падзея не чакае адпраўкі на сервер, каб лічыцца паказанай. Перад паказам пасля аўдыё — новае рашэнне па свежай пазіцыі, не выкананне старога GPS-callback. Пусты/састарэлы каталог або непрыдатная пазіцыя даюць адсутнасць прапановы, не аварыйны запыт прыватнага кантэнту.
