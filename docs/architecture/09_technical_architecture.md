...
7. Аўтазапуск немагчымы, калі `autoplay_suspended = true` або стан ≠ `Active`. Сцяг ставяць `UserPausedAudio`, `FocusLoss` і `Pause`; здымае **толькі** яўная каманда карыстальніка (`Resume`, `UserSelectedStop`, `UserSelectedStory`).
