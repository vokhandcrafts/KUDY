...
- `previews.json` — «асобна серыялізаваны публічны анонс locked-кропак» з `09`,
  разд. 5: толькі `stop_id`, `place_id`, `name`, `announce`. Дублюецца ў кожную
  локаль наўмысна: адзінаца дастаўкі — пласт `locale × tier` (09, разд. 4), таму
  кожны base-пласт мусіць быць самадастатковым разам з сваім `lock.json`.
